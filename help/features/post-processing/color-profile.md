---
helpx_url: "https://helpx.adobe.com/de/substance-3d-painter/features/post-processing/color-profile.html"
breadcrumb-title: ''
description: Erfahren Sie, wie Sie die Farbprofil-Nachbearbeitung in Substance 3D Painter verwenden, um Farbabstufung und LUT-Transformationen anzuwenden.
...
Substance 3D Painter ermöglicht die Zuweisung von **Farbprofilen** zu **Viewports** durch Laden von **LUT** Texturen.\
...
>  Substance 3D Painter verwendet **3D** LUTs, die als **2D-Textur** (Gleitend) mit einer beliebigen Auflösung gespeichert sind (Standardeinstellung: **2048x128 Pixel** ). Dies bedeutet, dass der Würfel, der die Farboperationen speichert, in Scheiben getrennt ist, die nebeneinander angezeigt werden. Weitere technische Details finden Sie im Artikel **GPU Gem** : 2<http://http.developer.nvidia.com/GPUGems2/gpugems2_chapter24.html>
...
* Wenn &quot;Farbprofil aktivieren&quot; **deaktiviert** ist, erfolgt das Rendern des Viewports in **sRGB** für die Materialansicht (und linear für einige bestimmte Kanäle).
...
Wenn eine LUT-Textur in den Ressourcensteckplatz geladen wird, wird sie verwendet, um das Rendern des Viewports zu bearbeiten, wenn sie sich im **Materialmodus** befindet.\
...
Substance 3D Painter verschiebt den Viewport auf **Linear**-Rendering, wenn **Farbprofil aktivieren** aktiviert ist. Dies bedeutet, dass eine LUT, wenn sie angewendet wird, Farben von einem linearen Profil in das gewünschte Ziel übersetzen muss.

### Methode 1: Ändern der Identitäts-LUT

Die Bearbeitung der Identitäts-LUT kann in einer Software erfolgen, die <b>32bits floating</b> textures unterstützt, z. B. <b>Substance 3D Designer</b>. Laden Sie die Identitäts-LUT als Ausgangspunkt herunter, um ein neues Profil zu erstellen:
...
### Neues Farbprofil importieren

Öffnen Sie einfach das Importfenster (oder ziehen Sie die LUT per Drag &amp; Drop in die Ablage). Stellen Sie beim Importieren der LUT-Textur in Substance 3D Painter sicher, dass Sie der neuen Ressource &quot;**colorlut** &quot; **usage** zuweisen. Andernfalls wird die Ressource nicht richtig im Regal angezeigt.
