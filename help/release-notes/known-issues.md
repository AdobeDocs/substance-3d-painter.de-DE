...
* `[Instancing]` `[Projection]` Wenn Sie eine Instanz in einem planaren Projekt auswählen, wird ein anderes planares Projekt auf einem anderen Textursatz ausgewählt.
...
* `[Layer stack]` In Maske kopieren wechselt zu Materialmodi
...
* `[Geometry mask]` Problem beim erneuten Importieren von Mesh und instanzierter Ebene
...
* Ungültige Mesh-Normalwerte mit einigen fbx
* Ansicht wird nicht aktualisiert, wenn der Kanal geändert wird, der von einer Ebene betroffen ist
* Projekte mit einem Textursatz werden im Grundfarbe-Solomodus erneut geöffnet
* Die Benutzeroberfläche der Kanalschaltfläche in den Material-/Malen-Eigenschaften kann unterbrochen werden.
...
* Das Verhalten der Schaltfläche &quot;Wiederherstellen&quot; interagiert nicht mit dem Sperrschlüssel in den Einstellungen der Kamera
* Beim Exportieren in Photoshop wird die Auswahl von Geometriemasken ignoriert.
* Die Steigung des Weichzeichners und der Verkrümmungsfilter hängen von der Auflösung des Textursatzes ab
* Karten ohne Namen werden außerhalb des Exportordners erstellt
* Schablone wird beim Ändern der Pinselvorgabe nicht aktualisiert
...
* `[Crash]` Das Klicken auf die Liste der Textursatz nach der fehlgeschlagenen Projekterstellung verursacht einen Absturz
* Absturz &quot;`[Crash]` Kritischer Fehler&quot;, wenn dasselbe Projekt zweimal geöffnet ist
* `[Crash]` &quot;Mesh exportieren&quot; auswählen, wenn der Mesh nicht geladen werden konnte
* `[Crash]` Klicken auf &quot;Malen beginnen&quot;, nachdem versucht wurde, ein altes Projekt zu öffnen
* `[Crash]` Absturz beim Erstellen sehr langer Texte im Menüband.
* `[Crash]` Rückkehr zum Malmodus nach Geräteverlust im Baking
* `[Crash]` Beenden Sie Painter nach dem Abbrechen des Kartenexports
* `[Crash]` Mesh mit einigen Sonderzeichen im Namen der Kamera wird exportiert
