...
**Substance 3D Painter 7.3** bietet mit den neuen Krümmungs- und Zylinderprojektionen für Füllebenen neue Möglichkeiten zur Texturierung von Netzen.

Freigabedatum: *13. Oktober 2021*

## Wichtigste Funktionen

### Neue Verkrümmungsprojektion

![](../../assets/proj-warp.jpg)

In dieser Version wird die neue 3D-Verkrümmungsprojektion für Füllebenen und Fülleffekte eingeführt. Diese Projektion ermöglicht es, eine Textur oder ein Bild mithilfe eines Deformationsgitters und steuerbarer Punkte zu verzerren.

* **Schnelle Einrichtung per Drag-and-Drop** Wählen Sie ein Material, ein Alpha, eine Textur oder ein Verfahren aus der Assets-Bibliothek aus, ziehen Sie es und legen Sie es auf den gewünschten Teil des Gitters ab (Tastenkombination **ALT** erforderlich für Materialien). Wenn das Element kein Material ist, wird ein Popup mit der Frage angezeigt, welchem Kanal es zugewiesen werden soll.\
  Sobald die Ebene erstellt wurde, wird die neue *Verkrümmungsprojektion* automatisch ausgewählt. Die Ebene verfügt über standardmäßige Steuerelemente für den 3D-Projektionsmodus, aber auch über den neuen Parameter *Projektionswarteschlange*, mit dem die Tiefe der Verkrümmungsprojektion (dargestellt durch grüne Tiefen als visuelle Warteschlange) festgelegt werden kann.\
  Sie können diesen Projektionsmodus auch manuell für jede Füllebene oder jeden Effekt auswählen, ohne ein Element per Drag &amp; Drop in den Viewport ziehen zu müssen.

  ![](../../assets/drop-viewport-warp.gif)

* **Automatische Platzierung mit dem Oberflächenwerkzeug** Wenn die neue Verkrümmungsebene erstellt wird, wird das Oberflächenwerkzeug automatisch ausgewählt. Auf diese Weise können Sie das Bild so verschieben, dass es jederzeit auf der Oberfläche des Gitters bleibt. Sie können jedoch jederzeit zu einem der anderen Manipulatoren wechseln und seine Übersetzung (Tastaturbefehl **W**), Drehung (Tastaturbefehl **E**) oder Skalierung (Tastaturbefehl **R**) anpassen. Um zum Oberflächenwerkzeug zurückzukehren, verwenden Sie den Tastaturbefehl **UMSCHALT + W**. Wenn Sie in den Modus *Eckpunkte bearbeiten* wechseln, ist das Oberflächenwerkzeug auch die Standardauswahl und richtet die Scheitelpunktbewegung an der Oberfläche des Gitters aus. Sie können jedoch das Oberflächenwerkzeug vorübergehend und schnell überschreiben, indem Sie **STRG** beibehalten. Dadurch können Sie den ausgewählten Punkt in eine beliebige Richtung verschieben, nicht nur auf der Oberfläche.

* **Leicht bearbeitbares Verkrümmungsraster** Nachdem die globale Platzierung des Bildes abgeschlossen ist, ist es auch möglich, das Verkrümmungsraster selbst zu bearbeiten, um eine höhere Präzision und Flexibilität zu erzielen. Um in den Rasterbearbeitungsmodus zu wechseln, können Sie entweder das neu hinzugefügte Verkrümmungsmenü oder den Tastaturbefehl **UMSCHALT + V** verwenden. Auf diese Weise können Sie vorhandene Scheitelpunkte des Rasters bearbeiten.\
  Sie können das Raster insgesamt gleichmäßig unterteilen, aber beachten Sie, dass die Scheitelpunkte, die Sie zuvor verschoben haben, auf ihre ursprüngliche Position zurückgesetzt werden. Die Rasterunterteilung kann über das neue Menü mit den Verkrümmungsoptionen vorgenommen werden.\
  Alternativ ist es möglich, individuell platzierte Splits hinzuzufügen, die nur bei Bedarf mehr Details ermöglichen würden. Um Teilungen hinzuzufügen, wähle eine der drei Optionen im Menü &quot;Verkrümmen&quot; aus: &quot;Kreuzen&quot;, &quot;Horizontal&quot; oder &quot;Vertikal&quot;. Wenn du einen Clip auswählst und mit dem Cursor auf die Verformen-Projektion zeigst und auf eine beliebige Stelle darin klickst, wird der Clip geteilt. Dies ändert nichts an der Position der vorhandenen Punkte.

  ![](../../assets/warp-split.gif)
* **Automatische Anpassung der Scheitelausrichtung** Standardmäßig werden die Tangenten einzelner Scheitelpunkte an die Gitteroberfläche angepasst, was bedeutet, dass sie immer korrekt in Bezug auf das Gitter ausgerichtet sind, unabhängig davon, wo sie gezogen werden. Diese Option für die automatische Tangente kann über eine neue Schaltfläche in der kontextabhängigen Symbolleiste deaktiviert werden. In diesem Fall bleibt die Ausrichtung jederzeit unverändert.

  ![](../../assets/warp-tangent-adjustment.gif)

Weitere Informationen zu den Einstellungen und Eigenschaften der Verkrümmungsprojektion finden Sie auf der [dedizierten Dokumentationsseite](../../painting/fill-projections/warp-projection.md).

### Neue Zylinderprojektion

![](../../assets/cylinder-proj.jpg)

Diese Version fügt eine zylindrische Projektionsmethode für Füllebenen und Fülleffekte hinzu. Mit der neuen Projektion können Sie ein Bild oder eine Textur um Objekte wie Spalten, Säulen oder mehr organische Formen wie die Arme einer Figur anpassen.

* **Ein Bild um ein Gitter legen**\
  Sie können ein Bild ganz einfach um eine zylindrische Oberfläche legen, indem Sie eine Füllebene oder einen Fülleffekt verwenden und *Zylindrische Projektion* in der Dropdownliste &quot;Projektion&quot; auswählen. Wenn das Bild nicht außerhalb des Projektions-Gizmos wiederholt werden muss, müssen Sie *Keine* für *UV-Umhüllung* und *Auf Form zugeschnitten* in *Form zuschneiden* auswählen, um sicherzustellen, dass Ihr Bild nicht außerhalb der Grenzen liegt. Dann müssen Sie nur den Manipulator verwenden, um die Projektion an die gewünschte Position anzupassen.
...
* **Neue 0-255 RGB-Werte**\
  Neben den bestehenden Möglichkeiten zur Eingabe von Farbwerten ermöglicht der verbesserte Farbwähler auch das Arbeiten mit 0-255 RGB-Werten. Diese Option ist verfügbar, wenn im Dropdownmenü des Schiebereglerabschnitts die Option *Gleitkommawerte* deaktiviert ist.
...
* **Verbesserungen beim Ziehen und Ablegen von Elementen**\
  Mit der Einführung der Verkrümmung wurde die Aufkleberfunktion, mit der Elemente aus der Bibliothek in den Viewport gezogen und abgelegt werden können, während die ALT-Taste beibehalten wird, überarbeitet. Wenn ein Aufkleber auf diese Weise erstellt wird, verwendet er nicht mehr die Planar-Projektion, sondern die Verkrümmen-Projektion. Die automatische Auswahl der Verkrümmen-Projektion sollte die Geschwindigkeit und Effizienz der Aufkleberanpassungen am Gitter verbessern.\
  Darüber hinaus ist es jetzt möglich, nicht nur Materialien, sondern auch bildartige Elemente in den Viewport zu ziehen und dort abzulegen. Bei der Auswahl eines Alpha-Elements, einer Textur oder eines prozeduralen Elements ist die Verwendung des ALT-Modifizierers nicht erforderlich. Es kann auf das Gitter abgelegt werden - das würde ein Menü auffordern, das die Möglichkeit bietet, auszuwählen, ob dieses Bild innerhalb einer Maske oder in einem der Kanäle der Ebene verwendet werden soll.

  ![](../../assets/improved-decal.gif)

* **Verbesserung des Plug-ins zum automatischen Speichern**\
  Das automatische Speichern wird nicht mehr bei längeren oder schwereren Vorgängen wie dem erneuten Laden des Gitters, dem Backen oder dem Exportieren ausgelöst.
...
* **Neue Funktionen in der Python-API**\
  Die Python-API hatte einige kürzlich hinzugefügte Funktionen, die es ermöglichen, das Gitter neu zu laden, Ressourcen zu aktualisieren sowie die Auflösung von UV-Kacheln über Skripterstellung festzulegen und abzufragen.

* **Substance-Engine-Update 8.3.0**\
  Neben einigen Korrekturen und allgemeinen Verbesserungen berücksichtigt dieses Substance-Engine-Update jetzt neue Diagrammtypen. Es ist auch möglich, die .sbsar-Dateiversion zu überprüfen, wodurch die Verwendung und das Herunterladen der entsprechenden Substance 3D Assets-Versionen verbessert werden sollte.

* **Substance 3D Assets wird vom CC-Desktop empfangen**\
  Sie können jetzt über die CC-Desktop-Applikation auf Substance 3D Assets wie Materialien, Atlanten und Aufkleber zugreifen. Außerdem können sie direkt an die Painter Library gesendet werden.
