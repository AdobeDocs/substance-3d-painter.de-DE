...
| --- | --- |
| **Ausrichtung** | Die Ausrichtung gibt an, wie ein Pinsel beim Malen nach 3D-Mesh ausgerichtet wird. |
| **Alpha** | Ein Alpha ist eine Maske, die zum Malen von Details oder komplexen Formen verwendet werden kann, z. B. ein Strichcode oder ein Logo. |
...
| **Kamera** | Die Kamera ist das Objekt, mit dem Sie Position und Blickrichtung in einem 3D- oder 2D-Viewport steuern können. |
| **Kanal** | Ein Kanal ist eine Textur mit einem bestimmten Verhalten. Einige Kanäle definieren die Farbgebung eines Materials, andere dienen der Steuerung des Verhaltenslichts auf einer Oberfläche. |
| **Klon** | Der Klon ist ein Werkzeug, mit dem ein Teil des Bildes/der Textur an einem anderen Ort repliziert wird. |
...
| **Diffusion** | Die Diffusion ist eine Möglichkeit, Informationen außerhalb der UV-Insel eines Meshs zu generieren. Die Methode funktioniert durch Ausbluten der letzten Pixel in der Nähe eines UV-Inselrands, wodurch verschwommene Farben erzeugt werden. |
| **Ausdehnung/Auffüllung** | Nach demselben Prinzip wie die Diffusion ist die Ausdehnung eine Möglichkeit, Informationen außerhalb der UV-Insel zu generieren, indem Pixelfarben erweitert werden. |
...
| **Umgebung** | Eine Umgebung ist ein Bild, das zur Berechnung der Beleuchtung einer Szene verwendet wird. In der Regel handelt es sich dabei um eine HDR. Textur, die eine Vielzahl von Farbinformationen darstellt. |
| **Export** | Beim Exportieren werden abgeflachte Texturen aus dem in der Anwendung realisierten Gemälde generiert. Die beim Exportieren erstellten Texturen können in anderen Anwendungen verwendet werden. |
...
| **Iray** | Iray ist ein von NVIDIA erstellter Pfad-Tracer-Renderer, mit dem eine realistische Beleuchtung auf den 3D-Mesh projiziert wird. Da es sich um einen erweiterten Renderer handelt, ist es für die Erstellung schöner Bilder gedacht und nicht für Echtzeitarbeiten. |
...
| **Verzögerte Mausbewegung** | Die verzögerte Mausbewegung ist ein Verhalten des Pinselwerkzeugs. Es verlangsamt den Pinselpfad, um die Präzision beim Malen zu erhöhen, und erzeugt eine Verzögerung/einen Versatz zwischen dem Mauszeiger und dem eigentlichen Malen. |
...
| **Perspektive** | Perspektive ist eine ungefähre Darstellung eines Objekts oder einer Szene, die das menschliche Auge auf einer ebenen Fläche (wie einem Bildschirm) sieht. Es ist eine Simulation von Tiefe und Maßstab. |
...
| **Shader** | Ein Shader definiert das Verhalten eines Materials, wenn es Beleuchtungsinformationen erhält. Einige Shader können einfach (z. B. eine Zehen-Schattierung) oder etwas komplexer (z. B. eine Haut-Schattierung, die eine leichte Absorption auf einer Oberfläche simuliert) sein. |
| **Regal** | Ein Regal ist der Speicherort in der Anwendung, an dem Ressourcen unterschiedlicher Art organisiert sind. Es kann von einfachen Bildern zu komplexeren Werkzeugen führen. |
| **Intelligente Maske** | Intelligente Masken verhalten sich wie intelligente Material. Anstatt jedoch Ebenen zu sein, handelt es sich um Effekte, die so definiert sind, dass sie nur Masken basierend auf dem aktuellen 3D-Mesh generieren. |
| **Intelligente Material** | Intelligentes Material ist eine Ebenengruppe, die als eine Datei gespeichert wird. Intelligente Material kann sich an jedes Projekt in Substance 3D Painter anpassen, sodass Materialien erstellt werden können, die sich je nach aktuellem 3D-Mesh ändern. |
...
| **Schablone** | Eine Schablone ist ein Bild, das am Bildschirm ausgerichtet und mit einer Kamera-Projektion zum Malen auf dem 3D-Mesh verwendet wird. |
| **Substance** | Eine Substance ist ein Dateiformat, mit dem Sie Texturen auf der Grundlage von Parametern (prozedurale Berechnungen) generieren können. Diese Parameter können geändert werden, um Variationen zu erzeugen. |
...
| **UDIM** | UDIM ist eine Möglichkeit, die UVs eines 3D-Mesh über mehrere Bereiche aufzuteilen, um die allgemeine Auflösung der Textur zu erhöhen. |
| **UV** | UV sind Informationen, die auf einem 3D-Mesh definiert sind und angeben, wie er zu einer flachen Form ausgeklappt werden kann. Diese Informationen werden verwendet, um eine Textur auf einen 3D-Mesh zu projizieren. Substance 3D Painter erlaubt nur das Malen im Bereich von 0-1, was der Größe einer Textur entspricht. Andere Bereiche werden nur über das UDIM-System unterstützt. |
| **VRam** | Der VRam ist der Speicher der GPU (Grafikkarte), die zum Speichern von Informationen und Textur bei der Berechnung verwendet wird. Je mehr VR, desto besser für die Arbeit mit Substance 3D Painter. |
