...
Im Substance Painter können Sie eigene Shader in *GLSL* schreiben. Wir erlauben Ihnen, nur einen *Teil* des Fragment-Shader zu schreiben, der manchmal als *Surface-Shader* bezeichnet wird. Lassen Sie uns ohne weitere Umschweife den &quot;Hello World&quot; Substance Painter Surface Shader vorstellen:
...
Wenn Sie dieses Snippet jetzt in einer Datei vom Typ *.glsl* speichern und in den Substance Painter laden, indem Sie es auf der Registerkarte &quot;Shader&quot; Ihres Regals ablegen, können Sie es jetzt verwenden und ein schönes gleichmäßiges Rosa auf Ihrem Mesh sehen.
...
In einem Shader gibt es gewöhnlich maßgeschneiderte Anpassungen. Dazu haben wir in den Shadern von Substance Painter eine Möglichkeit eingeführt, benutzerdefinierte Anpassungen festzulegen. Hier finden Sie eine vollständige Liste aller benutzerdefinierten Shader-Tweak-Typen :
...
* [lib-alpha.glsl](libraries-shader-api/lib-alpha-shader-api.md) : enthält Helfer für Deckkraft
* [lib-bayer.glsl](libraries-shader-api/lib-bayer-shader-api.md) : enthält Helfer für Bayermatrix
* [lib-define.glsl](libraries-shader-api/lib-defines-shader-api.md) : enthält nützliche mathematische Konstanten
* [lib-emissive.glsl](libraries-shader-api/lib-emissive-shader-api.md) : enthält Helfer für emissive-Eigenschaften
* [lib-env.glsl](libraries-shader-api/lib-env-shader-api.md) : enthält Helfer zum Umgebungs-Map
* [lib-normal.glsl](libraries-shader-api/lib-normal-shader-api.md) : enthält Normalen-Map-bezogene Helfer (und Height-Map-generierte Normalen-Map)
...
* [lib-pom.glsl](libraries-shader-api/lib-pom-shader-api.md) : enthält Helfer für die Parallaxenzuordnung von Verdeckungen
...
* [lib-sparse.glsl](libraries-shader-api/lib-sparse-shader-api.md) : enthält sichere Helfer für die Aufnahme von Texturen mit geringer Dichte
* [lib-sss.glsl](libraries-shader-api/lib-sss-shader-api.md) : enthält Volumenstreuung Helfer
...
* [lib-vectors.glsl](libraries-shader-api/lib-vectors-shader-api.md) : enthält gängige Helfer für Vektorgrafiken
...
* **benutzerdefinierte-Benutzeroberfläche**: Ersetzen Sie die Standardbenutzeroberfläche für Shader-Parameter durch eine benutzerdefinierte Ansicht, die als QML-Modul geschrieben wurde (siehe Skriptdokumentation). Der Pfad kann absolut oder relativ zu einem der Ordner &quot;*custom-ui*&quot; Ihres Regals sein.
...
Um einen Eindruck davon zu bekommen, was wie ein echter Shader aussieht, hier einige Beispiel-Shader, geordnet nach steigender Komplexität:

* [pixelated.glsl](shaders-shader-api/pixelated-shader-api.md) : ein verpixelnder Shader
* [toon.glsl](shaders-shader-api/toon-shader-api.md) : ein Shader
* [pbr-metal-raw.glsl](shaders-shader-api/pbr-metal-rough-shader-api.md) : der Standard-PBR-Shader, der in Substance Painter eingebettet ist

## Dynamische Materialüberlagerung

Bei der Dynamische Materialüberlagerung handelt es sich um einen spezifischen Workflow, bei dem Material in einem Shader zusammengemischt werden und der Benutzer blending mask im Substance Painter dynamisch bearbeiten kann. Um diesen Arbeitsablauf zu aktivieren, gibt es zwei neue Funktionen:

* Deklarieren bearbeitbarer Stapel aus einer Shader-Definition: [schichtweise\_deklarieren\_Stapel.glsl](parameters-shader-api/layering-declare-stacks-shader-api.md)
